...
export default async function ChargesTable() {
  ...
  // Pull last 24 invoices (about 2 years of monthly billing)
  const invoices = await stripe.invoices.list({
    customer: user.stripeCustomerId,
    limit: 24,
    expand: [
      "data.charge",
      "data.payment_intent.latest_charge", // extra path if charge isn’t directly populated
  ...
  const rows = invoices.data.map((inv) => {
    ...
    // Try to show the subscription/nickname if present
  ...
  // Sort newest first (Stripe already returns newest first, but ensure)
